...
CONCURRENCY = 10
RATE_LIMIT = 0.15  # seconds between requests (be kind to CDN)


async def download_file(client: httpx.AsyncClient, url: str, dest: Path, sem: asyncio.Semaphore) -> bool:
...
async def preload_thumbnails(client: httpx.AsyncClient, sem: asyncio.Semaphore, limit: int | None = None, all_games: bool = False):
    ...
    for i, game in enumerate(games, 1):
        ...
        ok = await download_file(client, url, dest, sem)
        if ok:
...
async def preload_images(client: httpx.AsyncClient, sem: asyncio.Semaphore, limit: int | None = None, all_games: bool = False):
...
async def run(do_thumbs: bool, do_images: bool, limit: int | None, all_games: bool, concurrency: int = CONCURRENCY):
    ...
    async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
        if do_thumbs:
            s, f = await preload_thumbnails(client, sem, limit, all_games)
            print(f"\nThumbnails done: success={s} fail={f}")

        if do_images:
            s, f = await preload_images(client, sem, limit, all_games)
...
def main():
    ...
    print(f"Preload config: thumbs={do_thumbs} images={do_images} limit={args.limit} all={args.all_games} concurrency={concurrency}")

    asyncio.run(run(do_thumbs, do_images, args.limit, args.all_games, concurrency))
