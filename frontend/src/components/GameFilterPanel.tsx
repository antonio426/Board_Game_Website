...
export interface Facets {
  ...
  playtime: { min: number; max: number; count: number }[];
  weight: { min: number; max: number; count: number }[];
}
...
export default function GameFilterPanel({ filters, onChange, facets, categories, mechanics }: Props) {
  ...
  const options = (vocabulary: TagVocabulary[], buckets: FacetBucket[] | undefined): ChipOption[] =>
    vocabulary.map((tag) => ({
      ...
      count: facets ? facetCount(buckets, tag.name) ?? 0 : undefined,
    }));
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {WEIGHT_BAND_KEYS.map((band, index) => {
              const count = facets?.weight
                .filter((bucket) => (band === "light" ? bucket.max <= 2 : band === "medium" ? bucket.min >= 2 && bucket.max <= 3.5 : bucket.min >= 3.5))
